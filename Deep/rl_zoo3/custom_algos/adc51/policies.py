...
class CategoricalNetwork(BasePolicy):
    ...
    def __init__(
        ...
        self.atoms_total = n_atoms * n_critics

        self.beta_expl = beta_expl

        self.categorical_nets = []
        ...
        for i in range(n_critics):
            ...
            complete_qf_net = nn.Sequential(features_extractor, qf_net)
            self.add_module(f"qf{i}", complete_qf_net)
            self.categorical_nets.append(complete_qf_net)
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        logit_nets = th.stack(tuple(qf(preprocessed_obs) for qf in self.categorical_nets), dim=1)

        return logit_nets.view(-1, self.n_critics, int(self.action_space.n), self.n_atoms)

    def _predict(self, observation: th.Tensor, deterministic: bool = True) -> th.Tensor:
        logits = self(observation)
        probs = nn.functional.softmax(logits, dim=-1)

        q_values = (probs * self.support).sum(dim=-1).mean(dim=1)

        # TODO adapt for categorical
        # exploration bonus
        # if not deterministic and self.beta_expl > 0:
        #     if self.n_critics > 2:
        #         ws_distances = th.abs(quantiles.unsqueeze(2) - quantiles.unsqueeze(1)).mean(dim=-2)

        #         mask = th.triu(th.ones(self.n_critics, self.n_critics), diagonal=1).bool()

        #         ws_distances = ws_distances[:, mask]

        #         mean_ws_distance = ws_distances.mean(dim=1)
        #     else:
        #         mean_ws_distance = th.abs(quantiles[:, 0] - quantiles[:, 1]).mean(dim=-2)

        #     q_values = q_values + self.beta_expl * mean_ws_distance
