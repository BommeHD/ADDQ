...
class QuantileNetwork(BasePolicy):
    ...
    def __init__(
        ...
        for i in range(n_critics):
            ...
            self.quantile_nets.append(complete_qf_net)
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        quantiles = th.stack(tuple(qf(preprocessed_obs) for qf in self.quantile_nets), dim=1)

        return quantiles.view(-1, self.n_critics, self.n_quantiles, int(self.action_space.n))

    def _predict(self, observation: th.Tensor, deterministic: bool = True) -> th.Tensor:
        quantiles = self(observation)

        q_values = quantiles.mean(dim=[1, 2])
