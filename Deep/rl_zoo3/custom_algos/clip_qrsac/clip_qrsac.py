...
from rl_zoo3.custom_algos.clip_qrsac.policies import Actor, CnnPolicy, Critic, MlpPolicy, MultiInputPolicy, TQCPolicy

SelfClip_QRSAC = TypeVar("SelfClip_QRSAC", bound="Clip_QRSAC")


class Clip_QRSAC(OffPolicyAlgorithm):
    """
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                min_indices = next_quantiles.mean(dim=-1).argmin(dim=-1)

                # td error with clip logic + entropy term
                target_quantiles = next_quantiles[th.arange(batch_size), min_indices] - ent_coef * next_log_prob.reshape(-1, 1)
