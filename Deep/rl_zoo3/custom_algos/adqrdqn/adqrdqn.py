...
from rl_zoo3.custom_algos.adqrdqn.policies import CnnPolicy, MlpPolicy, MultiInputPolicy, QRDQNPolicy, QuantileNetwork

SelfADQRDQN = TypeVar("SelfADQRDQN", bound="ADQRDQN")


class ADQRDQN(OffPolicyAlgorithm):
    """
    ...
    def _create_aliases(self) -> None:
        ...
        self.median_idx = (self.n_quantiles - 1) // 2  # account for zero-based indexing
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for i in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                sorted_current = current_quantiles.sort(dim=2).values

                vs = th.mean(
                    (sorted_current[:, :, :, :] - sorted_current[:, :, [self.median_idx], :]) ** 2, dim=[1, 2]
                )
                ...
                target_quantiles = replay_data.rewards + (1 - replay_data.dones) * self.gamma * (
                    betas * next_quantiles[th.arange(batch_size), random_indices]
                    + (1 - betas) * next_quantiles[th.arange(batch_size), 1 - random_indices]
                )
