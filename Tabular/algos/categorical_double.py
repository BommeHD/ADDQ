...
class CategoricalDouble(Algo):
    def __init__(self,
        ...
        """
        Initializes the Categorical Double Q Learning algorithm. The environment and policy along with their arguments are 
        used to model the Markov decision process. The learning rate schedule mode and if it should be applied statewise or 
        not is passed. Optionally, a manual initialization of the atom probabilities is possible.
    ...
    def step(self) -> Tuple[int,int,int,float,bool]:
        ...
        for atom_index in range(self.num_atoms):
            ...
            if target_atom_rel_index.is_integer():
                target_atom_probs[int(target_atom_rel_index)] += self.atom_probs[other_index][(next_state,max_action)][atom_index]
            else:
                lower = int(np.floor(target_atom_rel_index))
                upper = int(np.ceil(target_atom_rel_index))
                target_atom_probs[lower] += self.atom_probs[other_index][(next_state,max_action)][atom_index] * (upper - target_atom_rel_index)
                target_atom_probs[upper] += self.atom_probs[other_index][(next_state,max_action)][atom_index] * (target_atom_rel_index - lower)
