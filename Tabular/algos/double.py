...
class Double(Algo):
    def __init__(self,
        ...
        """
        Initializes the Double Q Learning algorithm. The environment and policy along with their arguments are used to 
        model the Markov decision process. The learning rate schedule mode and if it should be applied statewise or not 
        is passed. Optionally, a manual initialization of the Q functions is possible.
    ...
    def step(self) -> Tuple[int,int,int,float,bool]:
        ...
        # Update the Q function according to the Double Q Learning algorithm
        self.q_fct[index][(self.current_state,chosen_action)] = (1 - stepsize) * self.q_fct[index][(self.current_state,chosen_action)] + stepsize * (reward + (1 - t) * self.gamma * self.q_fct[other_index][(next_state,max_act)])
